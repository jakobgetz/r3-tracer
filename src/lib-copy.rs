...
use walrus::{
    ...
    ModuleConfig, ModuleTypes, TableId, ValType,
};
...
pub fn instrument_wasm(buffer: &[u8]) -> Result<Module> {
    ...
    let mut generator = Generator::new(trace_mem_id, mem_pointer, &module.types);
    module
        .funcs
        .iter_local_mut()
        .for_each(|(_, f)| ir::dfs_pre_order_mut(&mut generator, f, f.entry_block()));
    println!("here it works still");
    let result = module.emit_wasm_file("emit.wasm");
    match result {
        Ok(_) => println!("OK"),
...
impl<'a> VisitorMut for Generator<'a> {
    fn visit_instr_mut(&mut self, instr: &mut Instr, instr_loc: &mut walrus::InstrLocId) {
        let mut builder = FunctionBuilder::new(&mut self.types, &[ValType::I32], &[ValType::I32]);
        let mut seq = builder.func_body();
        let instr_clone = instr.clone();
        match instr_clone {
            Instr::Call(_) => {
                self.trace_call(&mut seq, instr_clone);
            }
            ...
            Instr::Load(load) => {
                ...
                self.increment_mem_pointer(&mut seq, 5 + byte_length as i32);
                let func = builder.finish(locals, &mut self.module.funcs);
                *instr = Instr::Call(Call { func });
            }
            Instr::Store(store) => {
...
impl<'a> Generator<'a> {
    fn new(trace_mem_id: MemoryId, mem_pointer: GlobalId, types: &'a ModuleTypes) -> Self {
        Self {
            trace_mem_id,
            mem_pointer,
            types,
